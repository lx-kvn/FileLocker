...
### 6.8 空／有東西兩種信封圖示，跟拖曳懸停預覽

`Envelope_Body_Empty.svg` 是使用者直接把 `Envelope_Body.svg` 裡烤進去的裝飾三角形圖示拿掉
做出來的版本，兩張圖除了那個圖示有沒有之外完全同尺寸同座標，所以切換只需要換 `img.src`，
不用調整任何位置：

```js
function render(){
  const hasFiles = pickedList.children.length > 0;
  bodyImg.src = hasFiles
    ? '../../assets/Envelope_Body.svg'
    : '../../assets/Envelope_Body_Empty.svg';
}
```
...
envelopeCanvas.addEventListener('mouseenter', () => {
  if (pickedList.children.length === 0) {
    bodyImg.src = '../../assets/Envelope_Body.svg';       // 提前預覽「有東西」

...
namespace FileLocker.App.Tests;

/// <summary>
/// 2026-08-09 這輪安全稽核發現 <see cref="PasswordLockerNativePipeServer"/> 原本完全信任連線端
/// 跟訊息內容——這裡不是一般的行為測試，而是把稽核發現的每一條攻擊流程直接寫成斷言，固定住
/// 修復後的行為，避免之後的改動又悄悄放寬回沒有防護的版本。
///
/// 每個測試案例用自己專屬、帶隨機後綴的管道名稱（見 <see cref="UniquePipeName"/>），一來避免
/// 跟同一台機器上真的在跑的 FileLocker.exe 搶同一個具名管道，二來避免測試案例之間互相干擾
/// （xunit 預設會平行跑同一個類別以外的測試）。<c>expectedClientExePath</c> 一律傳目前測試
/// 行程自己的模組路徑——測試裡的「連線端」就是這個測試行程本身，不是真正的 Native Host exe，
/// 這樣 VerifyClientIsExpectedHost 才會通過，除非某個測試刻意要驗證「連線端不符」的情況。
/// </summary>
public sealed class PasswordLockerNativePipeServerTests
{
    ...
    /// 這裡只需要驗證 PasswordLockerNativePipeServer 這一層「該不該轉發、轉發後怎麼處理回應」

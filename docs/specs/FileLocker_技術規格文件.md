...
- [1. 專案總覽](#1-專案總覽)
- [2. 系統架構](#2-系統架構)
- [3. 金鑰衍生與加密演算法](#3-金鑰衍生與加密演算法)
  - [3.1 密碼 → 金鑰（Argon2id + HKDF）](#31-密碼-金鑰argon2id-hkdf)
  - [3.2 內容加密（AES-256-GCM + 串流分塊）](#32-內容加密aes-256-gcm-串流分塊)
  - [3.3 檔案完整性與竄改防護](#33-檔案完整性與竄改防護)
- [4. 加密／解密流程（`LockService`）](#4-加密解密流程lockservice)
  - [4.1 公開方法一覽](#41-公開方法一覽)
  - [4.2 加密流程](#42-加密流程)
  - [4.3 四種解密路徑](#43-四種解密路徑)
  - [4.4 密碼驗證與其餘操作共用的核心](#44-密碼驗證與其餘操作共用的核心)
  - [4.5 批次操作限制](#45-批次操作限制)
- [5. 資料夾加密與單檔案分散式加密](#5-資料夾加密與單檔案分散式加密)
  - [5.1 巢狀 `.locked` 項目的處理](#51-巢狀-locked-項目的處理)
  - [5.2 暫存空間位置與清理流程](#52-暫存空間位置與清理流程)
  - [5.3 單檔案分散式加密（`.flocked`）](#53-單檔案分散式加密flocked)
  - [5.4 `.flocked` 檔案格式](#54-flocked-檔案格式)
  - [5.5 加密前的所需空間預估](#55-加密前的所需空間預估)
- [6. Passkey（Windows Hello）機制](#6-passkeywindows-hello機制)
  - [6.1 API](#61-api)
  - [6.2 內容金鑰包裝方式](#62-內容金鑰包裝方式)
  - [6.3 視窗焦點問題（`WindowFocusHelper`）](#63-視窗焦點問題windowfocushelper)
  - [6.4 密碼鎖定機制不適用於 Passkey](#64-密碼鎖定機制不適用於-passkey)
- [7. 恢復金鑰機制](#7-恢復金鑰機制)
  - [7.1 產生與顯示格式](#71-產生與顯示格式)
  - [7.2 顯示與保存的一次性設計](#72-顯示與保存的一次性設計)
- [8. 關鍵操作驗證機制（Critical Action）](#8-關鍵操作驗證機制critical-action)
  - [8.1 後端 API（`VaultProtocolHandlers`）](#81-後端-apivaultprotocolhandlers)
  - [8.2 保護等級分層與前端使用情境](#82-保護等級分層與前端使用情境)
  - [8.3 四套憑證的命名與遺失後果](#83-四套憑證的命名與遺失後果)
- [9. 密碼錯誤鎖定機制](#9-密碼錯誤鎖定機制)
- [10. Vault 與本機索引](#10-vault-與本機索引)
  - [10.1 `VaultManager`：Vault 檔案佈局](#101-vaultmanagervault-檔案佈局)
  - [10.2 `LockedItemMetadata`（`.meta.json` 內容）](#102-lockeditemmetadatametajson-內容)
  - [10.3 本機加速索引（`VaultIndexCache` + `VaultChangeWatcher`）](#103-本機加速索引vaultindexcache-vaultchangewatcher)
- [11. Vault 設定與雲端同步](#11-vault-設定與雲端同步)
  - [11.1 Vault 位置](#111-vault-位置)
  - [11.2 雲端同步的運作模式](#112-雲端同步的運作模式)
- [12. 安全性設計總覽](#12-安全性設計總覽)
  - [12.1 Vault 簽章金鑰的角色邊界](#121-vault-簽章金鑰的角色邊界)
  - [12.2 `.locked` 指標檔完整性（簽章機制）](#122-locked-指標檔完整性簽章機制)
  - [12.3 密碼遺失情境](#123-密碼遺失情境)
- [13. 前後端 IPC 協定](#13-前後端-ipc-協定)
  - [13.1 `useIpc.js` 封裝](#131-useipcjs-封裝)
  - [13.2 前端 → 後端訊息（`MainWindow.OnWebMessageReceived` case 對應表）](#132-前端-後端訊息mainwindowonwebmessagereceived-case-對應表)
  - [13.3 後端 → 前端訊息（`messageHandlers` 對應表，節錄關鍵分組）](#133-後端-前端訊息messagehandlers-對應表節錄關鍵分組)
  - [13.4 `VaultProtocolHandlers`：Protocol 層完整介面](#134-vaultprotocolhandlersprotocol-層完整介面)
- [14. GUI 設計](#14-gui-設計)
  - [14.1 視窗外觀（原生 WPF 層，`MainWindow.Chrome.cs`）](#141-視窗外觀原生-wpf-層mainwindowchromecs)
  - [14.2 設計系統](#142-設計系統)
  - [14.3 主視窗導覽與各畫面](#143-主視窗導覽與各畫面)
  - [14.4 密碼輸入小視窗（`PasswordPromptWindow`）](#144-密碼輸入小視窗passwordpromptwindow)
  - [14.5 自訂互動元件（取代原生瀏覽器對話框）](#145-自訂互動元件取代原生瀏覽器對話框)
  - [14.6 恢復金鑰顯示彈窗：整個 App 的簽名視覺元素](#146-恢復金鑰顯示彈窗整個-app-的簽名視覺元素)
  - [14.7 圖示](#147-圖示)
- [15. CLI 介面（`FileLocker.Cli`）](#15-cli-介面filelockercli)
- [16. 資源管理器整合機制](#16-資源管理器整合機制)
  - [16.1 為什麼批次（多選）加密需要正規 Shell Extension](#161-為什麼批次多選加密需要正規-shell-extension)
  - [16.2 Shell Extension（`FileLockerShellExtension.dll`）](#162-shell-extensionfilelockershellextensiondll)
  - [16.3 右鍵選單登錄：主要機制在 App 端自我註冊，不是安裝程式](#163-右鍵選單登錄主要機制在-app-端自我註冊不是安裝程式)
  - [16.4 `.locked` 副檔名關聯（已完成，由安裝程式設定檔處理）](#164-locked-副檔名關聯已完成由安裝程式設定檔處理)
  - [16.5 32/64 位元](#165-3264-位元)
- [17. 多語言](#17-多語言)
- [18. 版本相容性策略（以「不需要額外相容性程式碼」為原則）](#18-版本相容性策略以不需要額外相容性程式碼為原則)
- [19. 安裝程式打包](#19-安裝程式打包)
- [20. 開發進度總覽](#20-開發進度總覽)
- [21. 資料夾防護（Folder Guard）](#21-資料夾防護folder-guard)
  - [21.1 憑證模型](#211-憑證模型)
  - [21.2 ACL 機制](#212-acl-機制)
  - [21.3 Shell Extension 整合](#213-shell-extension-整合)
  - [21.4 上鎖／解鎖互動](#214-上鎖解鎖互動)
  - [21.5 與加密流程的互動](#215-與加密流程的互動)
  - [21.6 選配功能：雙擊已上鎖資料夾直接解鎖（`.lockfolder` 標記檔）](#216-選配功能雙擊已上鎖資料夾直接解鎖lockfolder-標記檔)
  - [21.7 選配功能：解鎖後閒置自動重新上鎖](#217-選配功能解鎖後閒置自動重新上鎖)
- [22. 軟體更新檢查](#22-軟體更新檢查)
- [23. 已知限制（非缺陷，為取捨或現有技術/流程的限制）](#23-已知限制非缺陷為取捨或現有技術流程的限制)
- [24. 待辦事項](#24-待辦事項)
  - [24.1 進行中](#241-進行中)
  - [24.2 已完成之待辦](#242-已完成之待辦)
...
- **PasswordVault 獨立化——設定頁「安裝密碼庫」按鈕的人工點擊驗證**：整條「查 Release → 挑資產 → 下載 → 解壓 → 切換生效 → 載入 → 建立主密碼」已用本體自己的程式碼打真實 GitHub API 驗證過（見 24.2 節），剩下的只有在正式安裝的環境裡實際點一次那顆按鈕，確認 UI 狀態轉換與「重啟才生效」的流程沒有意外。
...
- **CLI 獨立發布產物（CLI_setup／CLI_zip）**：規劃已定案（詳見 `CONTEXT.md`「CLI 獨立發布產物」詞條），讓只想要 CLI、不想裝完整 GUI 的使用者有更輕量的取得方式。v2.1.0 這輪首次手動打包並上傳到 GitHub Release 驗證過沒有意外的坑，已收進 `/release` skill 的自動化流程。

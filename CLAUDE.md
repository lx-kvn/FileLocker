...
修改既有功能或新增功能前，必須先評估此次變更是否會限制未來的擴充性，並優先選擇保留冗餘空間的設計。

**反面案例**：單檔案分散式加密實作中，`.flocked` 檔頭 Header 長度被寫死為固定長度。這導致未來若要在 Header 新增欄位，會與舊版本產生的 `.flocked` 檔案不相容，或必須額外編寫版本判斷邏輯來維持相容性。

**設計原則**：
- 涉及檔案格式、資料結構、API 介面等會被序列化或跨版本使用的設計，優先採用可擴充的結構（例如：長度可變欄位、版本號欄位、保留位元/位元組）
- 動手實作前，先反問自己：「如果未來要在這裡加東西，現在的設計會不會擋路？」
- 若某個決策明顯會犧牲未來擴充性以換取當下的方便，需明確告知使用者這個取捨，而非默默的繼續做下去

...
│   │   └── features/                        # 各功能各自的規劃訪談紀錄；只放「還活著」的規劃
│   │       └── 已完成/                     # 功能已落地、內容只剩歷史脈絡的規劃文件搬來這裡，
│   │                                       # 避免跟仍在進行中的規劃混在一起看不出哪份還要更新
...
- 篇幅較長（大致 150 行或超過約 10 個 `##`／`###` 章節）的文件要在開頭補一份目錄（`## 目錄`，含子章節），方便長文件內導覽；改動這份文件的章節結構（新增/刪除/改標題）時，目錄要跟著同步更新，不要讓目錄跟實際章節脫節。
